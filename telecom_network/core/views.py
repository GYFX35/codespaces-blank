...
from .models import Profile, Connection, Post, Conversation, ChatMessage, Game, AffiliateItem
from .serializers import (
    ...
    GameSerializer, AffiliateItemSerializer
)
    ...
    def create(self, request, *args, **kwargs):
        ...
        if existing_connection:
            if existing_connection.status == 'pending':
                return Response({"message": "Connection request already sent and is pending."}, status=http_status.HTTP_400_BAD_REQUEST)
            if existing_connection.status == 'accepted':
                return Response({"message": "You are already connected with this user."}, status=http_status.HTTP_400_BAD_REQUEST)
        ...
        if reverse_existing_connection:
            if reverse_existing_connection.status == 'pending':
                return Response({"message": f"{to_user.username} has already sent you a connection request. Please check your incoming requests."}, status=http_status.HTTP_400_BAD_REQUEST)
            if reverse_existing_connection.status == 'accepted':
                return Response({"message": "You are already connected with this user (connection initiated by them)."}, status=http_status.HTTP_400_BAD_REQUEST)
...
class ConnectionActionView(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if action == 'accept' and connection.to_user == request.user and connection.status == 'pending':
            ...
            return Response({"error": f"Cannot perform action '{action}' on this connection under current conditions."}, status=http_status.HTTP_400_BAD_REQUEST)
        connection.save()
